...
void CreateStandardMessage() {
  ...
  if(resetSendCounter > 0) {
    ...
    if(isCool) {
      ...
      // In heat mode with all zones closed the system expects a distinct zone byte.
      if(zoneByte == zoneOff) {
...
// Spectrolink frame checksum (output[7]).
//
// Reverse-engineered from captured frames: it is a CRC-8 with polynomial 0x62
// (reflected / right-shift form), initial value 0x00 and no final XOR. The seven
// data bytes output[0..6] are fed in REVERSE byte order (output[6] first ...
// output[0] last), LSB-first within each byte. This reproduces every validated
// hard-coded value the original sketch used to carry in lookup tables.
byte spectrolinkCrc() {
  byte crc = 0;
  for (int i = 6; i >= 0; i--) {
    crc ^= output[i];
    for (byte b = 0; b < 8; b++) {
      crc = (crc & 1) ? (byte)((crc >> 1) ^ 0x62) : (byte)(crc >> 1);
    }
  }
  return crc;
}
